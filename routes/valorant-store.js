...
router.get("/valorant/updateStore", limiter, async (req, res, _next) => {
  ...
  try {
    ...
    //Get Player Store
    await valorantApi
      .getPlayerStoreFront(rows[0].playerid)
      .then((response) => {
        store = response.data
      })
      .catch((error) => {
    ...
    if (status) {
      ...
      for (var i = 0; i < 4; i++) {
        ...
        //Get Tier
        const tierid = skindata.data.data.filter(
      ...
      await db.execute(
        "INSERT INTO `store-cache`(`userid`, `date`, `json`) VALUES (?,?,?) ON DUPLICATE KEY UPDATE `date` = VALUES(`date`), `json` = VALUES(`json`)",
        [
          rows[0].id,
          new Date().getFullYear() + "-" + m + "-" + new Date().getDate(),
          JSON.stringify(offersweapons),
        ]
      )
...
router.get("/valorant/getStore", async (req, res, _next) => {
  ...
  try {
    ...
    ;[rows, fields] = await db.execute(
      "SELECT * FROM `store-cache` WHERE `userid` = ? limit 1;",
      [rows[0].id]
    )
