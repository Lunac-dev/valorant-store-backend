...
router.get("/valorant/updateDashboard", limiter, async (req, res, _next) => {
  ...
  try {
    ...
    //Get Player Wallet
    await valorantApi
      .getPlayerWallet(rows[0].playerid)
      .then((response) => {
    ...
    if (status) {
      ...
      for (var i = 0; i < Object.keys(missionstmp).length; i++) {
        const mission = missionsdata.data.data.find(
          (v) => v.uuid == missionstmp[i]["ID"]
        )
        if (mission.type.includes("Daily")) {
          ...
          dailymissions.push({
            ...
            ExpirationTime: missionstmp[i]["ExpirationTime"],
            progress:
              missionstmp[i]["Objectives"][
                Object.keys(missionstmp[i]["Objectives"])[0]
              ],
          })
        } else {
          //Weekly
      ...
      await db.execute(
        "INSERT INTO `dashboard-cache`(`userid`, `date`, `json`) VALUES (?,?,?) ON DUPLICATE KEY UPDATE `date` = VALUES(`date`), `json` = VALUES(`json`)",
        [
          rows[0].id,
          new Date().getFullYear() + "-" + m + "-" + new Date().getDate(),
          JSON.stringify({ wallet: wallet, missions: missions }),
        ]
      )
...
router.get("/valorant/getDashboard", async (req, res, _next) => {
  ...
  try {
    ...
    ;[rows, fields] = await db.execute(
      "SELECT * FROM `dashboard-cache` WHERE `userid` = ?",
      [rows[0].id]
    )
