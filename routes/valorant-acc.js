...
router.get('/valorant/login', loginlimiter, async (req, res, _next) => {
  ...
  if (!await db.empty(discordid)) {
    ...
    let insert = await db.register(valorantApi.user_id, valorantApi.access_token, valorantApi.entitlements_token, valorantApi.ssidcookie, region, valorantApi.tdidcookie, discordid, relogin)
